...
void alu_eval(ALU *alu) {
	...
	/*compute word with 1 in each place that input src1 has a 0 and 0 in each place that input src1 has a 1*/

	case ALUOP_INV:
		result = ~A;
		break;

	/*compute unsigned int difference of unsigned int inputs src1 and src2*/
	...
	/*compute unsigned int sum of unsigned int input src1 and the number 1 (that is, compute result of incrementing src1 by 1)*/
	...
	/*compute word which has as bit i the value of bit i+1 in src1, then set bit 31 equal of result equal to bit 31 of src1 (that is, compute arithmetic shift right of src1)*/

	case ALUOP_SRA:
		result = (A >> 1) | (A & ((uint32_t) WORDWITHTOPBIT1ANDALLOTHERBITS0));
		break;

	/*compute word which has as bit i the value of bit i+1 in src1, then clear bit 31 (that is, compute logical shift right of src1)*/

	case ALUOP_SRL:
		result = (A >> 1) & ((uint32_t) WORDWITHTOPBIT0ANDALLOTHERBITS1);
		break;

	/*compute word which has as bit i the value of bit i-1 in src1 for i > 0 and 0 as bit 0 (that is, compute the arithmetic left shift of src1)*/

	case ALUOP_SLA:
		result = A << 1;
		break;

	/*compute word which has as bit i the value of bit i-1 in src1 for i > 0 and 0 as bit 0 (that is, compute the logical left shift of src1)*/
