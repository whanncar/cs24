...
void nextPC(ProgramCounter *pc) {
	...
	/*if the branch value on the bus is 1*/

	if(pin_read(pc->branch) == BRANCH){

		/*record the address value on the bus as the next program counter value*/

		next_pc = pin_read(pc->branch_addr);

	}

	/*if the branch value on the bus is 0*/

	else{

		/*record the current program counter value plus 1 as the next program counter value (that is, record that the next program counter is just the current one incremented)*/
