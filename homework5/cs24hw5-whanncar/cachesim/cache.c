...
unsigned char cache_read_byte(membase_t *mb, addr_t address) {
    ...
    p_line->most_recent_access_time = clock_tick();
...
void decompose_address(cache_t *p_cache, addr_t address,
    ...
    addr_t tag_mask, set_mask, offset_mask;
    ...
    /* Prepare the tag mask */
    tag_mask = 1 << (p_cache->sets_addr_bits + p_cache->block_offset_bits);
    tag_mask--;
    tag_mask = ~tag_mask;

    /* Prepare the set mask */
    set_mask = 1 << p_cache->block_offset_bits;
    set_mask--;
    set_mask = ~set_mask;
    set_mask ^= tag_mask;

    /* Prepare the offset mask */
    offset_mask = tag_mask | set_mask;
    offset_mask = ~offset_mask;

    /* Store the masked values */
    *tag = tag_mask & address;
    *set = set_mask & address;
    *offset = offset_mask & address;

    /* Bit shift the masked values */
    *tag = *tag >> (p_cache->sets_addr_bits + p_cache->block_offset_bits);
    *set = *set >> p_cache->block_offset_bits;
...
addr_t get_block_start_from_address(cache_t *p_cache, addr_t address) {
    ...
    return (address / p_cache->block_size) * p_cache->block_size;
}
...
addr_t get_offset_in_block(cache_t *p_cache, addr_t address) {
    ...
    return address % p_cache->block_size;
}
...
addr_t get_block_start_from_line_info(cache_t *p_cache,
    ...
    return tag * p_cache->num_sets * p_cache->block_size
                                + set_no * p_cache->block_size;
}
...
cacheline_t * find_line_in_set(cacheset_t *p_set, addr_t tag) {
    ...
    int i;
    cacheline_t *current_line;

#if DEBUG_CACHE
    printf(" * Finding line with tag %u in cache set:\n", tag);
#endif

    for (i = 0, current_line = p_set->cache_lines;
         i < p_set->num_lines;
         i++, current_line++) {
...
cacheline_t * choose_victim(cacheset_t *p_set) {
    ...
    int i;
    unsigned long long int age_of_LRU_line = -1;
    i_victim = 0;

    for (i = 0, victim = p_set->cache_lines;
         i < p_set->num_lines;
         i++, victim++) {

        /* If the line is invalid, return it */
        if (!(victim->valid)) {
            return victim;
        }
        /*
         * If this line is the least recently used among the lines checked
         * so far, store its index and update age_of_LRU_line to the last
         * time it was used
         */
        if (victim->most_recent_access_time < age_of_LRU_line) {
            i_victim = i;
            age_of_LRU_line = victim->most_recent_access_time;
        }
